...
**只读分享：** 分享窗口会说明快照有效期；本截图没有生成真实分享链接。

![象映笔记浅色模式下的分享窗口，可生成 7 天有效的只读快照；截图中没有真实分享链接。](docs/assets/screenshots/share-dialog-light.jpg)
...
- **按需分享：** 生成一份 7 天有效的只读快照，可以提前撤销；也可以把笔记和图片打包导出。
...
分享笔记时，象映笔记会生成一份只读快照。快照固定有效 7 天，原笔记之后的修改不会改变已经生成的内容；你可以在“已分享”或笔记的分享窗口中查看记录并撤销链接。请记住，任何拿到有效链接的人都能阅读其中的内容。
