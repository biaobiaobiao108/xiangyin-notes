...
- **SQLite 增量空间回收（Incremental Auto Vacuum）**：数据库采用 `incremental auto_vacuum` 模式。在彻底删除笔记或清空回收站后，系统会自动合并全文索引段（`optimize`）、增量回收未使用的空闲页（`incremental_vacuum`），并即时截断 WAL 日志（`wal_checkpoint(TRUNCATE)`），将磁盘空间物理归还给操作系统，防止数据库文件单向持续膨胀。
...
2. **彻底删除才会物理回收空间**：进入“回收站”选择**彻底删除**或**清空回收站**，系统才会执行真正的物理删除。此时系统会自动触发增量空间回收（Incremental Vacuum）、全文索引段优化以及 WAL 检查点截断，将空闲页立即归还给宿主机文件系统。
