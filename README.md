...
空数据库首次启动时，会自动执行唯一的基线迁移 `migrations/0001_baseline.sql` 完成初始化。

当前尚无生产数据，数据库结构已归一到单一基线文件，不再保留旧开发数据库的 schema 兼容分支或回填逻辑。若本地数据库来自旧版本，请先备份，再删除旧数据库并重新启动应用；需要保留内容时请先导出笔记。未来投入生产后，新增结构再使用新的增量迁移文件，不能修改已经应用过的迁移。
...
当前基线已包含笔记标题、双向链接目标、笔记本统计和列表排序所需的索引。
