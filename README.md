...
空数据库首次启动时，会自动按文件名顺序执行当前所有迁移完成初始化；当前包含 `0001_baseline.sql` 和 `0002_performance_indexes.sql`。

迁移历史由基线文件和后续增量迁移共同维护。已经应用过的迁移文件不能修改；如需保留旧数据库中的内容，请先备份，再执行升级迁移或导出需要的笔记。
...
`0002_performance_indexes.sql` 会增加笔记标题和双向链接目标的规范化索引，并为笔记本统计增加查询索引；迁移脚本会同时回填已有数据。
