...
- 色彩：暖白画布、白色表面、石墨文字；松柏军绿 `--accent` 是控件的主强调色。朱砂 `--h1-color` 仅用于正文 H1 或明确的错误/危险语义，不用于收藏和普通选中状态。
- 图标控件：默认使用低调灰色，悬浮变为石墨色，选中/激活使用绿色；收藏按钮未收藏显示空心星，已收藏显示绿色实心星，并使用 `aria-pressed` 表达状态。
- 状态提示：编辑器顶栏的保存、上传、冲突、错误等动态状态只显示固定尺寸图标；文字仅通过 `aria-label`、`title` 提供，不得因状态文字改变顶栏尺寸或挤压其他控件。
- 稳定布局：弹性或网格子项正确设置 `min-width: 0`；顶栏图标按钮保持固定触控尺寸且不可被 flex 压缩；标题、长字符串和状态变化不得造成横向溢出、重叠或跳变。
- 可访问性：优先使用原生语义控件，所有图标按钮有可读名称，保留可见 `:focus-visible`；状态不能只依赖颜色，还应结合图标、描边/填充或文字说明。
- 响应式：桌面、平板、窄屏手机都要检查；布局切换、浮层和抽屉不得遮挡编辑内容、阻断焦点或产生横向滚动。

### 视觉与组件设计规范

- 风格基调：秉持文人笔记的内敛、克制与纯净质感。保持暖白画布（`--canvas`）、白色表面（`--surface`）、石墨文字（`--ink`）与复古松柏军绿（`--accent`）的核心视觉系统，正文 H1 标题保留沉敛朱砂红（`--h1-color`）作为卷首印章点睛。
