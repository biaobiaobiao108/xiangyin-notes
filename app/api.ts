...
export const api = {
  ...
  createNotebook: (payload: { name: string; color?: string; icon?: string }) => request<{ notebook: Notebook }>("/api/notebooks", { method: "POST", body: JSON.stringify(payload) }),
  updateNotebook: (id: string, payload: { name?: string; color?: string; icon?: string }) => request<{ notebook: Notebook }>(`/api/notebooks/${id}`, { method: "PATCH", body: JSON.stringify(payload) }),
