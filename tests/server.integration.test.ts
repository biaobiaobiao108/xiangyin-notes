...
describe("Bun Server API", () => {
  ...
  test("automatically initializes a fresh database and does not rerun the baseline", async () => {
    ...
    expect(migrations.map((item) => item.name)).toEqual(["0001_baseline.sql", "0002_note_links.sql"]);
    ...
    expect(appliedMigrations.map((item) => item.name)).toEqual(["0001_baseline.sql", "0002_note_links.sql"]);
    reopened.close();
