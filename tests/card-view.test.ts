...
describe("card view & layout", () => {
  ...
  test("Safari card layout avoids fragmented shadows and keeps card content painted", async () => {
    ...
    expect(cardRule).toMatch(/border:\s*1px solid transparent/);
    ...
    const hoverCardRule = cssContent.match(/\.note-card:hover\s*\{([^}]*)\}/)?.[1] ?? "";
    expect(hoverCardRule).not.toMatch(/border(?:-color)?\s*:/);
    ...
    expect(cssContent).not.toMatch(/\.note-card\.is-selected\s*\{[^}]*border-color\s*:/);
